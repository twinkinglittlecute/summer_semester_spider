...
response = requests.get('http://www.chinapiao.com.cn/query.php')#get通常用来获取页面，点击链接() posed提交数据 delete head
# response(返回值)有state（200、404等）、header、content等字段，其中content字段是返回的内容，通常是html格式的。
html = etree.HTML(response.content)#解析代码
...
for x in low_price[:10]:
    print('价格:', x[4], '\t', x[2],'\t', x[0],'时间:',x[1])
print("中国演出票务网，在售最贵前10：")
for x in high_price[-10:]:
    print('价格:', x[3], '\t', x[2],'\t', x[0],'时间:',x[1])